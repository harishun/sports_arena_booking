...
// Handle profile update when the form in the overlay is submitted
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    $sql_check_duplicates = "SELECT username, e_mail FROM users WHERE (username = '$new_username' OR e_mail = '$email') AND username != '$original_username'";
    $result_duplicates = mysqli_query($conn, $sql_check_duplicates);

    if (mysqli_num_rows($result_duplicates) > 0) {
        $can_update = false;
        $duplicate_row = mysqli_fetch_assoc($result_duplicates);
        if ($duplicate_row['username'] === $new_username) {
            $message = "Error: That username is already taken by another user.";
        } elseif ($duplicate_row['e_mail'] === $email) {
            $message = "Error: That email address is already in use by another user.";
        }
    }
    ...
    if ($can_update) {
        ...
        if (mysqli_query($conn, $sql_update)) {
            ...
            $username = $new_username; // Update username for fetching new data
...
// Fetch current (or newly updated) profile details
$sql_fetch_user = "SELECT * FROM users WHERE username='$username'";
...
<body>
    ...
    <div class="overlay hidden">
        ...
        <form method="POST" action="profile.php">
            ...
            <div class="contact cardlet">
                <label>Email:</label>
                <br>
                <input type="email" name="e_mail" value="<?php echo htmlspecialchars($profile['e_mail']); ?>" required>
                <br>
    ...
    <div class="card">
        <h2>Profile Details
            <svg class="edit" onclick="toggle_overlay(document.querySelector('.overlay'))" id="edit" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#e3e3e3">
